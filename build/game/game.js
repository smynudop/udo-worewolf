...
const VillageSetting_1 = require("./VillageSetting");
const moment = require("moment");
class Game {
    constructor(io, data) {
        this.io = io;
        this.villageSetting = new VillageSetting_1.VillageSetting(data);
    ...
    fixInfo(data) {
        ...
        gameIO_1.GameIO.update(this.villageSetting.vno, data);
        this.log.add("system", "vinfo", { message: this.villageSetting.text() });
    }
    startRollcall() {
    ...
    start() {
        if (!this.canStart())
            return false;
        gameIO_1.GameIO.update(this.villageSetting.vno, { state: "playing" });
    ...
    casting() {
        var castlist = cast_1.castManager.jobList(this.villageSetting.casttype, this.players.num);
        if (!castlist)
            return false;
        ...
        var txt = cast_1.castManager.makeCastTxt(this.villageSetting.casttype, this.players.num);
    ...
    setnsec() {
        if (!this.villageSetting.time.nsec)
            return false;
    ...
    finish() {
        ...
        this.emitPlayerAll();
        this.emitAllLog();
        gameIO_1.GameIO.update(this.villageSetting.vno, { state: "finish" });
    ...
    pass(phase) {
        ...
        this.date.setTimer(next[phase], this.villageSetting.time[phase]);
    }
    emitAllLog() {
    ...
    listen() {
        this.io.on("connection", (socket) => {
            ...
            socket.on("enter", (data) => {
                if (this.players.in(userid))
                    return false;
                if (this.players.num >= this.villageSetting.capacity)
                    return false;
                data.userid = userid;
                data.socket = socket;
    ...
    close() {
        gameIO_1.GameIO.writeHTML(this.log.all(), this.players.list, this.villageSetting.villageInfo());
        gameIO_1.GameIO.update(this.villageSetting.vno, { state: "logged" });
    }
}
exports.Game = Game;
