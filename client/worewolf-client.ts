import {eachLog} from "../game/log"
import {IPlayerforClient} from "../game/player"
import {IVillageSetting} from "../game/VillageSetting"
import {ITalkDetail} from "../game/command"
...
export class Client{
    ...
    connect(): SocketIO.Socket{
        return io("/room-" + vno, {path: /localhost/.test(location.href) ? "/socket.io" : "/worewolf/socket.io"})
    }

    setMe(data:IPlayerforClient) {
        this.me = data
    ...
    listen(){
        ...
        this.socket.on("you", (data:IPlayerforClient) => {
            this.setMe(data)
        })
